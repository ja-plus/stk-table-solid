...
export function useVirtualScroll(
     ...
     * 无需修正时返回 null。
     */
    const virtualX_mergeColsInfo = createMemo(() => {
     ...
     * virtual + virtualX 模式下消费方走可视行分支，本 memo 直接返回 null，
     * 避免 O(rows × mergeCols) 的全量 mergeCells 调用浪费。
     */
    const virtualX_colMergeRangeFull = createMemo(() => {
     ...
     */
    const virtualX_colMergeRange = createMemo(() => {
     ...
     */
    const virtualX_colRange = createMemo(() => {
    ...
    const virtualX_offsetRight = createMemo(() => {
        ...
        const endIndex = isMultiLevelHeader() ? theadVirtualX().endIndex : virtualX_colRange().endIndex;
        let width = 0;
        const tableHeaderLastValue = tableHeaderLast();
