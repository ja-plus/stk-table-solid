 ...
 * 高亮单元格，行
 */
export function useHighlight(props: any, stkTableId: string, tableContainerRef: Accessor<HTMLDivElement | undefined>) {
    ...
    /** 持续时间 */
    const highlightDuration = createMemo(() => (config.duration ? config.duration * 1000 : HIGHLIGHT_DURATION));
    /** 高亮频率*/
    const highlightFrequency = createMemo(() => (config.fps && config.fps > 0 ? 1000 / config.fps : null));
    /** 高亮帧数（非帧率），用于 timing-function: steps() */
    const highlightSteps = createMemo(() => (highlightFrequency() ? Math.round(highlightDuration() / highlightFrequency()!) : null));
    /** 高亮开始 */
    const highlightFrom = createMemo(() => highlightColor[props.theme as 'light' | 'dark'].from);
    ...
    /** 高亮函数的默认参数 */
    const defaultHighlightDimOption = createMemo(() => {
    ...
    function highlightRowsInCssKeyframe(rowKeyValues: UniqKey[], className: string, duration: number) {
        ...
        if (needRepaint) {
            void tableContainerRef()?.offsetWidth; //强制浏览器重绘
        }
