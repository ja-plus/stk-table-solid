...
配置 `props.sortRemote` 为 `true`，这样就不会触发组件内部的排序逻辑。

点击表头后，会触发 `onSortChange` 事件，您可以在事件中发起 ajax 请求，然后重新对 `props.dataSource` 赋值，完成排序。
...
| defaultSort.silent | `boolean` | `false` | 是否禁止触发 `onSortChange` 事件。 |
